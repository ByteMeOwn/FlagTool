...
def load_flags():
    r = [(k, int(v, 0)) for k, v in FCONF["item"].items()]
    e = [(k, int(v, 0)) for k, v in FCONF["enchant"].items()]
    c = [
        (s.split("_", 1)[1], [(k, FCONF[s][k]) for k in FCONF[s]])
        for s in FCONF.sections()
        if s.startswith("classe_")
    ]
    return r, e, c
...
is_hex = (
    lambda v: all(
        ...
        for c in (v[2:] if v.lower().startswith("0x") else v)
    )
    and v != ""
)
...
class Tab(tk.Frame):
    ...
    def mk_cell(self, p, txt, val, b=False):
        f = tk.Frame(
            ...
            highlightthickness=D.get("celula_borda", 1),
            cursor="hand2",
            width=D.get("celula_largura", 140),
            height=D.get("celula_altura", 36),
        )
        f.pack_propagate(False)
        ft = (F[0], F[1], "bold") if b else F
        l = tk.Label(
            f, text=txt, font=ft, fg=C["texto"], bg=C["quadro_normal"], anchor="center"
        )
        l.pack(
            expand=True,
            fill=tk.BOTH,
            padx=D.get("celula_padx", 6),
            pady=D.get("celula_pady", 6),
        )
        for w in [f, l]:
            w.bind("<Button-1>", lambda e, v=val: self.tog(v))
            w.bind(
                "<Enter>", lambda e, fr=f: fr.config(highlightbackground=C["destaque"])
            )
            w.bind(
                "<Leave>",
                lambda e, fr=f, vl=val: fr.config(
                    highlightbackground=(
                        C["quadro_selec"] if vl in self.s else C["borda"]
                    )
                ),
            )
        self.cs[val] = f
    ...
    def __init__(self, p, cb):
        ...
        for i, (n, v) in enumerate(R):
            ...
            c.grid(
                ...
                column=i // nr,
                sticky="nsew",
                padx=D.get("celula_padding", 3),
                pady=D.get("celula_padding", 3),
            )
        for col in range(nc):
...
class App(tk.Tk):
    ...
    def mk_toolbar(self):
        ...
        c.pack(
            expand=True, padx=D.get("toolbar_padx", 8), pady=D.get("toolbar_pady", 6)
        )
        tk.Label(
            c, text="Modo:", font=(F[0], 9, "bold"), fg=C["destaque"], bg=C["header"]
        ...
        for txt, key in [
            ...
            btn = tk.Button(
                c,
                text=txt,
                command=lambda k=key: self.sw_tab(k),
                font=(F[0], 9, "bold"),
                bg=C["botao"],
                fg=C["botao_txt"],
                activebackground=C["botao_ativo"],
                activeforeground=C["botao_ativo_txt"],
                relief="flat",
                bd=0,
                width=D.get("botao_largura", 80) // 7,
                pady=D.get("botao_pady", 6),
                cursor="hand2",
            )
    ...
    def mk_statusbar(self):
        ...
        l.pack(
            side=tk.LEFT,
            fill="y",
            padx=D.get("statusbar_padx", 8),
            pady=D.get("statusbar_pady", 6),
        )
        tk.Label(
            l,
            text="Resultado:",
        ...
        self.lbl_val = tk.Label(
            l, text="0", font=(F[0], 10, "bold"), fg=C["texto"], bg=C["header"]
        )
        self.lbl_val.pack(side=tk.LEFT, padx=(0, D.get("espacamento_resultado", 12)))
        tk.Label(l, text="Valor:", font=(F[0], 9), fg=C["texto"], bg=C["header"]).pack(
        ...
        self.ent = tk.Entry(
            ...
            font=(F[0], 9),
            justify="center",
            width=D.get("entry_largura", 18),
            bg=C["card"],
            fg=C["texto"],
            insertbackground=C["destaque"],
            relief="flat",
            bd=1,
        )
        self.ent.pack(side=tk.LEFT, ipady=D.get("entry_pady", 3))
    ...
    def upd_res(self, tot):
        if self.cur in ["itens", "enchants"]:
            self.lbl_val.config(text=f"{tot}")
        else:
            self.lbl_val.config(text=f"{i2h(tot)}")
        self.ent.delete(0, tk.END)
        if tot != 0:
            self.ent.insert(
                0, f"{tot}" if self.cur in ["itens", "enchants"] else i2h(tot)
            )
    ...
    def cpy(self):
        v = (
            str(self.tabs[self.cur].t)
            if self.cur in ["itens", "enchants"]
            else i2h(self.tabs[self.cur].t)
        )
