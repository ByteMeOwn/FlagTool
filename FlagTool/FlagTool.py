...
class App(tk.Tk):
    ...
    def mk_statusbar(self):
        ...
        self.lbl_val=tk.Label(l,text="0",font=(F[0],10,"bold"),fg=C["texto"],bg=C["header"])
        ...
        self.ent=tk.Entry(l,font=(F[0],9),justify="center",width=D.get("entry_largura",18),bg=C["card"],fg=C["texto"],insertbackground=C["destaque"],relief="flat",bd=1)
    ...
    def upd_res(self,tot):
        ...
        if tot!=0:
            self.ent.insert(0,f"{tot}"if self.cur in["itens","enchants"]else i2h(tot))
    def chk(self):
